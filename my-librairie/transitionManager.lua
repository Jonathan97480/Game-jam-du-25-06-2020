...
local DefaultScript = {
  durationOut = 0.35, durationIn = 0.35, maskInput = true,
  easingOut = ease, easingIn = ease,
...
local function startPhase(dir)
  t.dir=dir; t.time=0; t.dur=(dir=="out" and (currentScript.durationOut or 0.3)) or (currentScript.durationIn or 0.3)
...
function Transition.play(options)
  opts=options or {}; local from=sceneManager.current
  ctx={ fromScene=from, toScene=opts.target, params=opts.params, w=love.graphics.getWidth(), h=love.graphics.getHeight() }
...
function Transition.update(dt)
  ...
  elseif state=="IN" then
    if donePhase then endPhase("in"); state="DONE" end
  end
end

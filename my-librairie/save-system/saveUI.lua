...
local function log(level, message)
    if _G.globalFunction and _G.globalFunction.log then
        _G.globalFunction.log[level](string.format("[SaveUI] %s", message))
    else
...
function saveUI.hide()
    ...
    if _G.hud then
        _G.hud.removeByTag("save_dialog")
    end
...
function saveUI.refreshSaveList()
    local SaveManager = _G.SaveManager or require("my-librairie.save-system.saveManager")
...
function saveUI.createInterface()
    ...
    _G.hud.addPanel("save_background", {
        layer = "background",
        x = 0,
        y = 0,
...
function saveUI.createSlotItem(save, x, y, index)
    -- Panneau du slot
    _G.hud.addPanel("save_slot_" .. index, {
        layer = UI_LAYER,
        x = x,
        y = y,
    ...
    _G.hud.addLabel("save_slot_title_" .. index, {
        layer = UI_LAYER,
        x = x + 10,
        y = y + 5,
        text = slotText,
